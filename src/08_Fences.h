  ...
  public:
    ...
    bool isDone(bool b = false) {
        ...
        for (int i = 0; i < buttons->size(); i++) {
            ...
            // if(!b->isPlaying || !b->on)continue; // if b is not playing or
            // not on, dont check.
    ...
    void update() {
        ...
        if (!isDone()) { // freeze when has reached...
            ...
            for (int i = 0; i < buttons->size(); i++) {
                ...
                buttons->at(i).updateFences(co->attraction * attractionDivider,
                                            co->gravity);
        ...
        if (moveEscalators) {
            ...
            for (int i = 0; i < escalators.size(); i++) {
                int floor = movingPolys[i].getBoundingBox().y +
                            movingPolys[i].getBoundingBox().height;

                float escalatorSpeed =
                    co->escalatorSpeed * float(1920 - floor) * 0.001;
                float ypos = abs(sin(time * escalatorSpeed)) * floor;

                escalators[i]->setPosition(escalators[i]->getPosition().x,
                                           ypos);
            }
        }
    }
    ...
    void begin(ofxBox2d *world = nullptr) {
        ...
        for (int i = 0; i < buttons->size(); i++) {
            if (buttons->at(rTickOrder[i]).isPlaying) {
                ...
                buttons->at(rTickOrder[i])
                    .setPosition(ofRandom(d.width - 50) + d.x + 25,
                                 ofRandom(d.height - 50) + d.y + 25);
                buttons->at(rTickOrder[i]).isWinner = false;
                buttons->at(rTickOrder[i]).getOutOfPolys(solidPolys);

                count++;
                if (count > 4)
                    d = dispensorPoly[0].getBoundingBox();
                if (count > 12)
                    d = dispensorPoly[1].getBoundingBox();
                if (count > 20)
                    d = dispensorPoly[2].getBoundingBox();
            }
        }
        for (int i = 0; i < dispensorPoly.size(); i++) {
            ofImage im;
            im.load("img/specialAssets/08_EscalatorDispensor-0" +
                    ofToString(i + 1) + ".png");
    ...
    void createDispensor(vector<ofPolyline> polys, float d = 0., float f = 0.,
                         float b = 0.) {
    ...
    void createPit(ofxBox2d *world, ofPolyline polys) {
        ...
        if (polys.getVertices().size() > 3 &&
            (rect.width < 1900 && rect.height < 1070)) {

            shared_ptr<ofxBox2dRect> r =
    ...
    void createScene(ofxBox2d *world, vector<ofPolyline> polys) {
        ...
        // cout << ofToString(polyShapes.size())+ " fences size in " +
        // co->sMap[co->sceneNumber]<< endl;
    }
};
