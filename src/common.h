...
class commonFunctions {
  public:
    ...
    vector<string> transformToCollumn(string str, int w,
        ...
        for (int i = 0; i < str.length(); i++) {
            ...
            appending.append(c);
            if (font->getStringBoundingBox(appending, 0, 0).width > w &&
                (c == " " || c == "-")) {
    ...
    string drawCollumn(vector<string> s, int x, int y,
                       ofxTrueTypeFontUC *font) {
        // ofPushMatrix();
        // ofTranslate(x,y);
    ...
    void test(ofxTrueTypeFontUC *font, int x, int y) {
        font->drawString("test", x, y);
    }

    string encode(string input, const string &inputEncoding,
    ...
    void drawGridLetters(int t, int b, ofxTrueTypeFontUC *f) {
        for (int i = 0; i < t; i++) {
            // return ofPoint(table * 70 + int(table/4)*20 + (1920 -( 70*12+20*3
            // + 70) ), row * 70 + 70);
            ofPushMatrix();
            ofTranslate(
                i * 70 + int(i / 4) * 20 + (1920 - (70 * 12 + 20 * 3 + 70)) -
                    f->getStringBoundingBox(ofToString(i + 1), 0, 0).width / 2,
                f->getLineHeight() + 30);
        ...
        for (int i = 0; i < b; i++) {
            ...
            // + 70) ), row * 70 + 70);
            ofPushMatrix();
            ofTranslate((1920 - (70 * 12 + 20 * 3 + 70)) - 90,
                        i * 70 + 140 + (70 - f->getLineHeight()) / 2);
    ...
    vector<ofPolyline> getPolyline(ofxSVG svg, bool doble = false) {
        ...
        for (int j = 0; j < svg.getNumPath(); j++) {
            ...
            vector<ofPolyline> &lines =
                const_cast<vector<ofPolyline> &>(p.getOutline());
