            ...
            if (buttons->at(i).isPlaying) {
                for (int j = i + 1; j < buttons->size(); j++) {
                    if (buttons->at(j).isPlaying) {
                        if (buttons->at(i).symbolInt ==
                                buttons->at(j).symbolInt &&
                            buttons->at(i).colorInt ==
                                buttons->at(j).colorInt) {
                            drawCircles[i] = true;
                            drawCircles[j] = true;
                            isDifferent = false;
                            break;
    ...
    void draw() {
        ...
        for (int i = 0; i < 6; i++)
            co->font_small->drawString(
                ofToString((i + 1) * 2 - (teamNumber * -1 + 1)),
                leftSide - 125 * 2, topSide + i * vSpace + 10);

        for (int i = 0; i < 6; i++)
            co->font_small->drawString(letters[i], leftSide + hSpace * i - 10,
                                       topSide - 100);
