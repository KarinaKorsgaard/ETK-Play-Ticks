...
class Button {
  public:
    ...
    void setup(int _row, int _table, int _teamNum, string _address,
               string _secondAdress, float val, float size, ofxBox2d *world) {
        ID = _row;
        table = _table;
        address = _address;
        ...
        box2Dcircle.get()->setup(world->getWorld(), ofRandom(200, 500),
                                 ofRandom(200, 500), radius);
    ...
    void drawDebug(ofRectangle rect = ofRectangle(0, 0, 1920, 1080)) {
        ...
        if (isPlaying) {
            ...
            ofTranslate(p);
            if (on)
                ofSetColor(ofColor::royalBlue, 100);
            else
                ofSetColor(ofColor::orangeRed, 20);
            ofDrawCircle(0, 0, 25);
            ofSetColor(255);
            ofDrawBitmapString(
                ofToString(table + 1) + " :" + ofToString(ID + 1), -10, 20);
            ofPopMatrix();
    ...
    void draw(bool transform = true, float scale = 1.f) {
        ...
        if (isPlaying) {
            ...
            img->at(charImg).draw(-radiusUsed, -radiusUsed, radiusUsed * 2,
                                  radiusUsed * 2);

            symbol->draw(-radiusUsed, -radiusUsed, radiusUsed * 2,
                         radiusUsed * 2);
            ofFill();
    ...
    void update(float attraction, bool doubleSize = true,
                ofRectangle rect = ofRectangle(0, 0, 1920, 1080)) {
        ...
        if (isPlaying && !isDead()) {
            ...
            ofRectangle r =
                ofRectangle(multiX + rect.x, rect.y, rect.width, rect.height);
    ...
    void updateFences(float attraction, float gravity, bool doubleSize = true,
                      ofRectangle rect = ofRectangle(0, 0, 1920, 1080)) {
        box2Dcircle->setVelocity(0, 0);
    ...
    void moveBackToBoard(ofRectangle r) {
        ...
        if (getPos().x < r.x)
            setPosition(r.x + radius, getPos().y);
        if (getPos().x > (r.width + r.x))
            setPosition((r.width + r.x) - radius, getPos().y);

        if (getPos().y < 0)
            setPosition(getPos().x, radius);
        if (getPos().y > 1080)
            setPosition(getPos().x, 1080 - radius);
    }
            ...
            if (lastPosForPolys == getPos()) {
                ...
                for (int i = 0; i < polys.size(); i++) {
                    ...
                    if (polys[i].inside(getPos())) {
                        ...
                        setPosition(newPos);
                        cout << "new pos " << getPos().x << " " << getPos().y
                             << endl;
    ...
    bool isDead() { return value == 0.0; }

    ofPoint getGridPos(int t, int i) {
        return ofPoint((teamNumber * 1920) + (i * 140 + 140), t * 70 + 140);
    }

    ofVec2f getPosRaw() { return ofVec2f(x * 1920.0f, y * 1080.0f); }
    ofVec2f getPos() { return box2Dcircle->getPosition(); }
    ofVec2f getBiquadPos() { return filterLowPass.value(); }
    ofVec3f getRawData() { return ofVec3f(x, y, r_raw); }
    void setRotation(float _r) { r = _r; }
    float getRotation() { return r; }
    float getRotationDelta() { return delta_r; }
    void setArmSwap(float a) { armSwapper = a; }

    void drawb2d() { box2Dcircle->draw(); }
