...
class AverageMaze : public Scene {
  ...
  public:
    ...
    void update() {
        ...
        if (!isDone()) {
            for (int i = 0; i < buttons->size(); i++) {
                buttons->at(i).update(
                    co->attraction); // the is on is checked in button!

                if (buttons->at(i).isPlaying) { // its on, not dead. (if it is
                                                // not playing, it can not be
                                                // on)
            ...
            if (indx > 0)
                average->addAttractionPoint(ofPoint(x / indx, y / indx),
                                            co->attraction);
    ...
    void begin(ofxBox2d *world = nullptr) {
        ...
        average.get()->setup(world->getWorld(), 1920 - 120 + addX, 1080 - 120,
                             25);
        // average->setRadius(45.f);
        for (int i = 0; i < buttons->size(); i++) {
            if (buttons->at(i).isPlaying) {

                if (buttons->at(i).getPosRaw().y > 0)
                    buttons->at(i).setPosition(
                        buttons->at(i).getPosRaw().x + addX, 1080 - 100);
                else
                    buttons->at(i).setPosition(ofRandom(100, 1920 - 100) + addX,
                                               1080 - 100);
            }
        }
