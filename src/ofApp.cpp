...
void ofApp::setup() {
    ...
    for (int i = 0; i < 12; i++) {
        co.characterSymbols[i].load("img/symbols/marker-" +
                                    ofToString(i + 1, 2, '0') + ".png");
    ...
    for (int i = 0; i < NUM_TABLES; i++) {
        ...
        tableIndx = i;
        if (i + 1 > NUM_TABLES / 2)
            tableIndx -= NUM_TABLES / 2;

        for (int j = 0; j < BUTTONS_PR_TABLE; j++) {

            Button b = *new Button;
            b.setup(j, i, teamNum, getAdress(0, i, j), getAdress(1, i, j),
                    startVal, startRad, &box2d);
            ...
            sd->buttonID =
                indx - (teamNum * ((BUTTONS_PR_TABLE * NUM_TABLES) / 2));
            sd->teamID = teamNum;
            sd->bHit = false;
            sd->isPlaying = false;
...
void ofApp::update() {
    ...
    if (co.sMap[co.sceneNumber] == "Fight") {
        ...
        fightBall.update(f1->pad_normal, f1->velocity_pad, f2->pad_normal,
                         f2->velocity_pad);
    }
...
void ofApp::draw() {
    ...
    if (co.sMap[co.sceneNumber] == "GroundGame")
        groundVideo.draw(0, 0, 1920, 1080);

    if (co.sMap[co.sceneNumber] == "Trail") {
        Trail *trail1 =
            static_cast<Trail *>(teams[0].scenes[co.sMap[co.sceneNumber]]);
        Trail *trail2 =
            static_cast<Trail *>(teams[1].scenes[co.sMap[co.sceneNumber]]);

        trailShader.begin();
        trailShader.setUniform2f("u_resolution", 1920 * 2, 1080);
        trailShader.setUniform2f("u_mask1", trail1->filter.value().x,
                                 trail1->filter.value().y);

        trailShader.setUniform2f("u_mask2", trail2->filter.value().x,
                                 trail2->filter.value().y);
        trailShader.setUniform1f("u_radius1", co.trailRadius[0]);
        trailShader.setUniform1f("u_radius2", co.trailRadius[1]);

        trailShader.setUniform2f("u_beginLight1", trail1->trailStart.x,
                                 trail1->trailStart.y);
        trailShader.setUniform2f("u_beginLight2", trail2->trailStart.x,
                                 trail2->trailStart.y);
 ...
 //   }
    ...
    if (!co.startScene) {
        ...
        //        int h = scoreMessages[co.sceneNumber].size() *
        //        font_small.getLineHeight();
        //
        //        if(scoreMessages[co.sceneNumber].size() > 0){
        //            vector<string> s;
        //
        //            s =
        //            cc.replace(scoreMessages[co.sceneNumber],"timekey",cc.timeToString(teams[0].time));
        //            for(int i = 0; i< s.size();i++){
        //                font_small.drawString(s[i], (1920 - 1280) / 2,
        //                (1080-h)/2+i*font_small.getLineHeight());
        //            }
        //            s =
        //            cc.replace(scoreMessages[co.sceneNumber],"timekey",cc.timeToString(teams[1].time));
        //            for(int i = 0; i< s.size();i++){
        //                font_small.drawString(s[i],1920 + (1920 - 1280) / 2,
        //                (1080-h)/2+i*font_small.getLineHeight());
        //            }
        //            //cc.drawCollumn( s ,1920 + (1920 - 1280) / 2, (1080-h)/2
        //            , &font_small);
        //        }
    }
    ...
    // ofDrawBitmapString("version "+ofToString(VERSION), 10, gui.getHeight() +
    // 30);
    for (int i = 0; i < NUM_TABLES; i++) {
        ...
        ofSetColor(255);
        ofDrawBitmapString(ofToString(i + 1), gui.getWidth() * 2 + 40 + 20,
                           i * 20 + 20 + 15);
    }
}
//--------------------------------------------------------------
void ofApp::handleSceneChange() {
    ...
    if (resent != -1) {
        ...
        co.delayPlayTime =
            float(ofToInt(ofSplitString(fullstring, " : ")[1])) / 1000.f;
        ...
        if (co.sMap[co.sceneNumber] == "Maze" ||
            co.sMap[co.sceneNumber] == "AverageMaze" ||
            co.sMap[co.sceneNumber] == "Fences") {
            box2d.disableEvents();
        } else {
            box2d.enableEvents();
        }
        ...
        if (co.sMap[co.sceneNumber] == "GroundGame") {
            if (!groundVideo.isLoaded()) {
                ...
                groundPixels.allocate(groundVideo.getWidth(),
                                      groundVideo.getHeight(), GL_RGB);
            }
        ...
        if (co.sMap[p_sceneNumber] == "Factories") {
            ...
            unemployedMessage1 =
                ("team 1 : table " + ofToString(co.isUnemployed[0].x + 1) +
                 " id: " + letters[co.isUnemployed[0].y]);
            unemployedMessage2 =
                ("team 2 : table " + ofToString(co.isUnemployed[1].x + 1) +
                 " id: " + letters[co.isUnemployed[1].y]);
        }
...
void ofApp::refill(int team, float timef) {
    ...
    for (int i = 0; i < teams[team].buttons.size(); i++) {
        ...
        if (b->isPlaying) {
            ...
            b->setValue(
                CLAMP(ease(t, oldValue, newValue, co.refillTime), 0, newValue));
    ...
    if (t > co.refillTime && team == 0)
        co.refill1 = false;
    if (t > co.refillTime && team == 1)
        co.refill2 = false;
}
//--------------------------------------------------------------
void ofApp::exit() {
    ...
    if (co.logReport) {
        string path = ofToString("reports/" +
                                 ofGetTimestampString("%m-%d_%H-%M") + ".txt");
        ofFile newFile(ofToDataPath(path),
                       ofFile::WriteOnly); // file doesn't exist yet
        newFile.create();
...
void ofApp::updateGroundGame() {
    ...
    for (int t = 0; t < 2; t++) {
        for (int i = 0; i < teams[t].buttons.size(); i++) {
            ...
            if (groundPixels.isAllocated()) {
                ...
                if (b->isDead() && b->isPlaying)
                    deadCounter++;

                if (deadCounter ==
                    co.numPresentButtons[0] + co.numPresentButtons[1] - 1) {
    ...
    if (!isGroundDone && co.startMovement)
        groundVideo.update();
}
            ...
            for (int t = 0; t < 2; t++) {
                for (int i = 0; i < teams[t].buttons.size(); i++) {
                    ...
                    if (m.getAddress() == b->secondAdress) {
                        ...
                        float x = m.getArgType(0) == 102
                                      ? m.getArgAsFloat(0)
                                      : float(m.getArgAsInt32(0));
                        float y = m.getArgType(1) == 102
                                      ? m.getArgAsFloat(1)
                                      : float(m.getArgAsInt32(1));
                        float z = m.getArgType(2) == 102
                                      ? m.getArgAsFloat(2)
                        ...
                        if (b->on && !b->isPlaying) {
                            b->isPlaying = true;
                            ButtonData *bd =
                                (ButtonData *)b->box2Dcircle.get()->getData();
                            bd->isPlaying = true;

                            ofVec2f p = ofVec2f(x * 1920 + b->teamNumber * 1920,
                                                y * 1080);

                            if (co.sMap[co.sceneNumber] == "Fences" ||
                                co.sMap[co.sceneNumber] == "AverageMaze")
                                p.y = 1000;
                            if (co.sMap[co.sceneNumber] == "Maze")
                                p.y = 60;
                            if (co.sMap[co.sceneNumber] == "Representative")
                                p.x = 100;
                            if (co.sMap[co.sceneNumber] == "Factories")
                                p = ofVec2f(1920 / 2 + b->teamNumber * 1920,
                                            1080 / 2);
    ...
    if (ofFile::doesFileExist("restart.xml")) {
        ...
        for (int u = 0; u < 2; u++) {
            for (int i = 0; i < 36; i++) {
                ...
                teams[u].buttons.at(i).isPlaying = xml.getValue(
                    team + ofToString(i) + "isPlaying", b->isPlaying);
                teams[u].buttons.at(i).symbolInt =
                    CLAMP(xml.getValue(team + ofToString(i) + "symbol", 0), 0,
                          co.characterSymbols.size() - 1);
                ...
                Design *designColor =
                    static_cast<Design *>(teams[u].scenes["Design"]);
                teams[u].buttons.at(i).color = designColor->colors[b->colorInt];
                teams[u].buttons.at(i).symbol =
                    &co.characterSymbols[b->symbolInt];
            }
        }
...
void ofApp::contactStart(ofxBox2dContactArgs &e) {
    if (e.a != NULL && e.b != NULL) {
        //        if(co.sMap[co.sceneNumber] == "Fight"){
        //            if(e.a->GetType() != b2Shape::e_circle && e.a->GetType()!=
        //            b2Shape::e_edge  && e.b->GetType() == b2Shape::e_circle) {
        //
        //                RacketData * racketData =
        //                (RacketData*)e.a->GetBody()->GetUserData(); ButtonData
        //                * buttonData =
        //                (ButtonData*)e.b->GetBody()->GetUserData();
        //
        //
