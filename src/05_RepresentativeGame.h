...
class Representative : public Scene {
  ...
  public:
    ...
    bool isDone(bool b = false) { return done; }
    ...
    void updateIsDone() {
        ...
        if (!doneFormation) {
            ...
            doneFormation =
                buttonsOutside.size() == spots.size() ? true : false;

            if (!doneFormation) {
                for (int s = 0; s < spots.size(); s++) {
                    tempTaken[s] = false;
                    for (int i = 0; i < buttonsOutside.size(); i++) {
                        if (getDist(spots[s].pos, buttons->at(buttonsOutside[i])
                                                      .getBiquadPos()) < 200) {
                            tempTaken[s] = true;
                        }
                    }
            ...
            if (doneFormation) {
                ...
                for (int i = 0; i < buttonsOutside.size(); i++) {
                    for (int s = 0; s < spots.size(); s++) {
                        ...
                        if (!spots[s].isTaken &&
                            getDist(spots[s].pos, buttons->at(buttonsOutside[i])
                                                      .getBiquadPos()) < 200) {
        ...
        if (doneFormation) {
            ...
            for (int b = 0; b < buttons->size(); b++) {
                if (buttons->at(b).isPlaying &&
                    buttons->at(b).getBiquadPos().x > frontHall &&
                    !buttons->at(b).isWinner) {
                    count++;
                }
            }
            ...
            for (int b = 0; b < buttons->size(); b++) {
                ...
                if (buttons->at(b).getBiquadPos().x > doorLimit + 10 &&
                    !buttons->at(b).isWinner) {
    ...
    void update() {
        ...
        for (int i = 0; i < doors.size(); i++) {
            if (doneFormation && !breakFormation) {
                if (abs(doors[i]->getPosition().y - originalPoints[i].y) <
                    100) {
                    int up = i * 2 - 1;
                    up *= -1;
                    doors[i]->setPosition(doors[i]->getPosition().x,
                                          doors[i]->getPosition().y - up);
            ...
            } else if (abs(doors[i]->getPosition().y - originalPoints[i].y) >
                       6) {
                int up = i * 2 - 1;
                ofxOscMessage m;
    ...
    void draw() {
        ...
        if (overlay.isAllocated())
            overlay.draw(1920 * teamNumber, 0, 1920, 1080);
    };
